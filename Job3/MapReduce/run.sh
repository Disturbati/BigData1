...
if [ $? -eq 0 ] && [ $5 -eq 3 ] ; then
    ...
    if [ $? -eq 0 ] ; then
        $HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/streaming/hadoop-streaming-3.3.4.jar \
            -mapper $PWD/$mapper3 \
            -reducer $PWD/$reducer3 \
            -input /user/$USER/output/$final_output \
            -output /user/$USER/output/$final_output2
    else
...
if [ $5 -eq 3 ]; then
    $HADOOP_HOME/bin/hdfs dfs -rm -R /user/$USER/output/$output
    $HADOOP_HOME/bin/hdfs dfs -rm -R /user/$USER/output/$final_output
fi