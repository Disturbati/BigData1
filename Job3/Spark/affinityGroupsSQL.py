...
def get_combinations(values):
    list = []
    list.extend(itertools.combinations(values, 3))
    return list

# get all the possible combinations of the products affinity
couple_df = couple_df.repartition("userId1", "userId2") \
    .withColumn("productsAffinity", explode(get_combinations("productsAffinity")))
...
group_df = user_df.groupBy("productsAffinity") \
    .agg(collect_set("userId").alias("usersAffinity")) \
